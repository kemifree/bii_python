...
class Base_file(object):
    ...
    #########################读取文件
    ##读取浏览来源数据####################################################################
    def read_pv(self,path,sheetname,business_name,skiprows):
        for root,dirs,files in os.walk(path):##文件夹的路径
            if files:   ##判断是否有文件
                for file_name in files:  ##循环文件的名称
                    #if '.xls' in file_name:  ##判断以xlsx结尾的文件是否在文件名称中
                    if 'xls.crdownload' in file_name:
                        ...
                        table_sheet[u'店铺']=business_name
                        ##判断表中是否有日期
                        date =u'日期'
                        if  date in table_sheet.columns:
                            table_sheet['date_temp'] = table_sheet[date]
                            del table_sheet[date]
                            table_sheet[date] = re.search(r'\d{4}-\d{2}-\d{2}',path).group(0)
                        else :
                            table_sheet[u'日期'] = re.search(r'\d{4}-\d{2}-\d{2}',path).group(0)
    ...
    def login_taobao_pv(self,username,password):
